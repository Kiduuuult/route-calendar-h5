...
公开浏览合作岩馆的已发布换线安排。项目包含移动端 React 页面及只读 BFF；浏览器不包含、也不会请求飞书凭证或 Base 信息。
...
2. 复制 `.env.example` 为 `.env`，只在服务端填写 `LARK_APP_ID`、`LARK_APP_SECRET`、`LARK_APP_TOKEN`、`LARK_TABLE_ID`、`LARK_VIEW_ID`。
3. 为应用申请 Base 只读权限与“下载云文档中的图片和附件”权限，并将“换线发布”Base 授权给该应用。
4. BFF 自动申请并缓存短期 `tenant_access_token`，仅请求“换线发布”的“对外页面数据源”视图。

不得读取或返回岩馆联系人、对外展示状态、历史主字段“岩馆名称”，也不得将任何飞书 Token、表 ID、字段 ID 写入前端包。
